...
if __name__ == "__main__":
    ...
    train_x, val_x, train_y, val_y = create_dataset(train_path, test_path,
        random_seed=random_seed, save=True)

    dataset = {}
    dataset["train"] = TextDataset(train_path, target_column=commons.target_column_name, normalize=normalize,
        balance=balance)
    dataset["val"] = TextDataset(val_path, target_column=commons.target_column_name, normalize=normalize,
        balance=False)
    ...
    results_folder = model_pytorch.train_model(model, dataset, batch_size, optimizer, scheduler, epochs,
